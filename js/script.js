...
// ── Comprimentos em unidades de medida reais ──
const ESCALA_COMPRIMENTO = [
  { v: 9.4607e15, sing: 'ano-luz',        plur: 'anos-luz'        },
  { v: 1.495978707e11, sing: 'unidade astronômica', plur: 'unidades astronômicas' },
  { v: 1e3,   sing: 'quilômetro', plur: 'quilômetros' },
  { v: 1,     sing: 'metro',      plur: 'metros'      },
  { v: 1e-2,  sing: 'centímetro', plur: 'centímetros' },
  { v: 1e-3,  sing: 'milímetro',  plur: 'milímetros'  },
  { v: 1e-6,  sing: 'micrômetro', plur: 'micrômetros' },
  { v: 1e-9,  sing: 'nanômetro',  plur: 'nanômetros'  },
  { v: 1e-12, sing: 'picômetro',  plur: 'picômetros'  },
  { v: 1e-15, sing: 'femtômetro', plur: 'femtômetros' },
  { v: 1e-18, sing: 'attômetro',  plur: 'attômetros'  },
  { v: 1e-21, sing: 'zeptômetro', plur: 'zeptômetros' },
  { v: 1e-24, sing: 'yoctômetro', plur: 'yoctômetros' },
  { v: 1e-27, sing: 'rontômetro', plur: 'rontômetros' },
  { v: 1e-30, sing: 'quectômetro', plur: 'quectômetros' }
];

function fmtComprimento(metros) {
  if (!isFinite(metros) || metros <= 0) return '0 metros';

  for (const u of ESCALA_COMPRIMENTO) {
    if (metros >= u.v) {
      const val = metros / u.v;
      if (val >= 1e6) return `${fmt(val)} de ${u.plur}`;
      const valStr = val.toLocaleString('pt-BR', { maximumFractionDigits: 3 });
      const ehSingular = Math.abs(Math.round(val * 1000) / 1000 - 1) < 1e-9;
      return `${valStr} ${ehSingular ? u.sing : u.plur}`;
    }
  }

  const ultima = ESCALA_COMPRIMENTO[ESCALA_COMPRIMENTO.length - 1];
  return `${decimalCompleto(metros / ultima.v)} ${ultima.plur}`;
}

// Comparação com objetos do dia a dia, para quem não conhece física
function compararTamanho(metros) {
  if (!isFinite(metros) || metros <= 0) return '';
  const refs = [
    { v: 1e-17, txt: 'bilhões de vezes menor que um próton' },
    { v: 1e-14, txt: 'mais ou menos do tamanho de um próton' },
    { v: 1e-12, txt: 'do tamanho do núcleo de um átomo' },
    { v: 1e-9,  txt: 'do tamanho de um átomo' },
    { v: 1e-7,  txt: 'do tamanho de uma molécula de DNA' },
    { v: 1e-5,  txt: 'do tamanho de um vírus ou de uma bactéria' },
    { v: 1e-3,  txt: 'do tamanho de um grão de areia' },
    { v: 1e-1,  txt: 'do tamanho de uma moeda ou de uma caneta' },
    { v: 1e1,   txt: 'do tamanho de uma pessoa ou de um carro' },
    { v: 1e3,   txt: 'do tamanho de um prédio bem alto' },
    { v: 1e5,   txt: 'do tamanho de uma cidade' },
    { v: 1e7,   txt: 'do tamanho de um país' },
    { v: 1e9,   txt: 'maior que o planeta Terra' },
    { v: 1e11,  txt: 'maior que o Sol' }
  ];
  for (const r of refs) {
    if (metros < r.v) return r.txt;
  }
  return 'maior que a distância da Terra ao Sol';
}

function linhaComprimento(metros) {
  return `${fmtComprimento(metros)}<br><span style="color:#8FA3B8; font-size:12px;">${compararTamanho(metros)}</span>`;
}
...
`<span style="color:#90E0EF; font-weight:500;">Raio inserido</span>
  ${fmtComprimento(r)}
  <span style="color:#8FA3B8; font-size:13px;">${compararTamanho(r)}</span>
...
function calcularComObjeto() {
  ...
  const label     = usouRaioCustom
    ? `buraco negro com raio ${fmtComprimento(raioCustom)}`
...
<span style="color:#90E0EF; font-weight:500;">Buraco negro alvo</span>  (${label})
  ...
  Raio de Schwarzschild: ${fmtComprimento(raioAlvo)}
  <span style="color:#8FA3B8; font-size:13px;">${compararTamanho(raioAlvo)}</span>
...
function renderTabela() {
  ...
  tbody.innerHTML = dados.map(obj => {
    ...
    return `<tr>
      ...
      <td class="td-mono">${linhaComprimento(raioProp)}</td>
